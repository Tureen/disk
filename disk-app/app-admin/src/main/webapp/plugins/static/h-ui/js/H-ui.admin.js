...
function Hui_admin_tab(obj){
	$(".Hui-aside").find('a').removeAttr('id','_selected');
	if($(obj).attr('_href')){
		$(obj).attr("id",'_selected');
		var bStop=false;
		var bStopIndex=0;
...
/*
	...
	h		弹出层高度（缺省调默认值）
*/
function layer_show_ajax(title,url,w,h){
	if (title == null || title == '') {
		title=false;
	};
	if (url == null || url == '') {
	...
	layer.open({
		...
		shade:0.4,
		title: title,
		content: getAndOpenPage(url)
	});
}

function getAndOpenPage(url,data){
