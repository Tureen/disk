$(function(){
	 ...
	 * 添加关联
	 */
	$(".ac_contact").live("click", function(){
		...
		$.ajax({
            ...
            success: function(data){
     	        if (data.code == 1000) { 
	       	        layer.msg(i18n_global_add_success,{icon: 1,time:1500});
		        	setTimeout(function(){refresh();}, 1500);
     	        }else{
     	        	layer.msg(i18n_global_add_fail,{icon: 2,time:1500});
     	        }
            }
		});
	});
	...
	function showOperBtn(fileArray){
		if(fileArray.length > 0 ){
			showAction($(".ac_contact"));
		} else{
			hideAction($(".ac_contact"));
		}
	}
