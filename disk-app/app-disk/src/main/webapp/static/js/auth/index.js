$(function(){
	...
	$(".user_share_tip").live("click",function(){
        ...
        $(this).parents(".share_con").siblings().find(".dropbox").fadeOut();
	...
	$(".select_down p,.topmenu_down p").live("click",function(e){
       ...
       $(this).parents(".share_con").siblings().find(".select_down div").slideUp();//遍历
	...
	$(".share_box2").click(function(){
		...
		if(count < 3){
			...
			layer.msg(i18n_add_limit_three_options,{icon: 5,time:2000});
		}
	});
...
function getSelectName(data){
	...
	//empIdArray不重复元素下标
	var indexArray = uniqueArray(empIdArray);
	...
	for(var index in empNameArray){
		if(indexArray.indexOf(index*1)!=-1){
	...
	data.parent().find("#empNumber").html(indexArray.length);
