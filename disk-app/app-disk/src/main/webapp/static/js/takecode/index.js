...
$(function(){
     ...
     */
  			...
  			$.ajax({
  	   	        ...
  	   	        success: function(result){
  	   		        ...
  	   		        if (result.code == 1000) {
  	   		        	...
  	   		        	layer.msg(result.codeInfo,{icon: 5,time:2000});
  	   		        }
  	   	        }
  	   		 });
  		}, 500);
 	...
 	/*$(".previewFile").on("click", function(){
 		...
 		$.ajax({
			url: basepath+"/takecode/preview?takeCode="+$("#takecodeindex").val(),
	        dataType:"json",
	        type: "POST",
	     	data: {fileId: fileId},
	        async: false, 
 	...
 	$(".previewFile").live("click", function(){
		...
		if(bool){
			layer.open({
                ...
                maxmin:true,
                content: fileServiceUrl + "/diskApi/onlinePreviewTxt?fileId="+ fileId + "&takeCode=" + $("#takecodeindex").val()
                ,btn: [close]
            });
		} else {
			//word 等相关文档
    ...
    $('.remove_qx').on('click', function(){
    	...
    	if(data.takecodes.length == 0){
    		layer.msg(i18n_select_extract,{icon: 5,time:1000});
	   		 return false;
	   	}
    	//构建移除展示
