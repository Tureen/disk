...
$(function(){
	...
	$(".array_h").click(function(){
		var data_url = basepath + "/bshare/index?type=1";
		var queryName = $("#queryName").val();
		if(queryName != null && queryName.length > 0){
			data_url = basepath + "/bshare/listkey?type=1" +"&queryName=" + encodeURI(encodeURI(queryName));
		} else {
			var folderId = $("#parentId").val();
			if(folderId != null && folderId != 0 && folderId.length > 0 ){
				var employeeId = $("#employeeId").val();
				var upId = $("#upId").val();
				data_url = basepath + "/bshare/indexinto/"+upId+"/"+folderId+"/"+employeeId+"?type=1" ;
			}
		}
	...
	$(".array_v").click(function(){
		var data_url = basepath + "/bshare/index?type=0";
		var queryName = $("#queryName").val();
		if(queryName != null && queryName.length > 0){
			data_url = basepath + "/bshare/listkey?type=0" +"&queryName=" + encodeURI(encodeURI(queryName));
		} else {
			var folderId = $("#parentId").val();
			if(folderId != null && folderId != 0 && folderId.length > 0 ){
				var employeeId = $("#employeeId").val();
				var upId = $("#upId").val();
				data_url = basepath + "/bshare/indexinto/"+upId+"/"+folderId+"/"+employeeId+"?type=0" ;
			}
		}
		window.location.href = data_url;
	});
   ...
   $("i[name='folderCheck']").each(function(){
	     var checkClass = $(this).attr('class');
	   	 if(checkClass != undefined &&　checkClass.indexOf('yes') != -1){
	   	    ...
	   	    //当前文件夹
	   		if(selectFolderId == folderId){
	   			bool = false;
	   		}
	   	    //父文件夹
	   		if(selectFolderId == pFolderId){
	   			bool = false;
	   		}
	   		//父文件夹
