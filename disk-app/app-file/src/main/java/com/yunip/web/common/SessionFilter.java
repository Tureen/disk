...
public class SessionFilter implements Filter {
    ...
    public void doFilter(ServletRequest arg0, ServletResponse arg1,
        ...
        if(isVerification(path)){
            UploadUser uploadUser = (UploadUser) request.getSession().getAttribute(Constant.UPLOADUSER_IN_SESSION);
            if(uploadUser == null){
                ...
                if("XMLHttpRequest".equals(requestType) && accept.contains("json")){
                    ...
                    error.put("code", FileServersException.SFYZSX.getCode());
                    error.put("codeInfo", FileServersException.SFYZSX.getMsg());
                    String errorJson = JsonUtils.getJSONString(error);
                    response.getOutputStream().write(errorJson.getBytes("UTF-8"));  
                    response.setContentType("text/json; charset=UTF-8");  
                    return;
                }else if(contentType != null && contentType.contains("multipart/form-data")){//判断是否为文件上传
                    if(path.indexOf("/upload/onlineEditUpLoad") != -1){//在线编辑上传提示
                        response.setContentType("text/json; charset=UTF-8"); 
                        response.getOutputStream().write(LocalLanguageHelper.getI18nValue(I18nContant.I18N_FILE_OPERATION_TIMEOUT, request).getBytes("UTF-8"));  
                        return;
                    }else{
                    ...
                    //跳转至提示界面
                    String diskUrl = SystemContant.DISK_SERVICE_URL;
                    request.setAttribute("msg", LocalLanguageHelper.getI18nValue(I18nContant.I18N_FILE_LOGIN_TIMEOUT_1, request) + "<a href='" + diskUrl + "'>" + LocalLanguageHelper.getI18nValue(I18nContant.I18N_FILE_LOGIN_TIMEOUT_2, request) + "</a>");
                    if(path.indexOf("/authorize/toUploadPage") == -1){
                        ...
                        request.getRequestDispatcher("/WEB-INF/view/common/sessionout_load.jsp").forward(request,response);
                    }
                    return;
                }
            }
