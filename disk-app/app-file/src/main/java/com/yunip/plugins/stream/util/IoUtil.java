...
public class IoUtil {
	...
	public static File getFile(String filename, String token) throws IOException {
		...
		File f = new File(Configurations.getFileRepository(token) + File.separator + name);
	...
	public static File getTokenedFile(String key) throws IOException {
		...
		File f = new File(Configurations.getFileRepository(key) + File.separator + key);
		if (!f.getParentFile().exists())
			f.getParentFile().mkdirs();
		if (!f.exists())
			f.createNewFile();
	...
	public static Range parseRange(HttpServletRequest req) throws IOException {
		String range = req.getHeader(StreamConstant.CONTENT_RANGE_HEADER);
	...
	public static long streaming(InputStream in, String key, String fileName) throws IOException {
		...
		try {
			...
			int read = 0;
			final byte[] bytes = new byte[StreamConstant.FORMDATA_BUFFER_LENGTH];
		...
		File dst = IoUtil.getFile(fileName, key);
		dst.delete();
		f.renameTo(dst);
