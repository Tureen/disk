...
(function(){
	var Provider, nIdCount = 0, aOtherBrowsers = ["Maxthon", "SE 2.X", "QQBrowser"],
		...
		sCellFileTemplate = '<b class="stream-uploading-ico"></b>' +
					...
					'	<a class="stream-cancel" href="javascript:void(0)">' + i18n_global_delete + '</a>' +
					'	<span class="stream-process-bar"><span style="width: 0%;"></span></span>' +
					'	<span class="stream-percent">0%</span>' +
					'</div>' +
					'<div class="stream-cell-infos">' +
					'	<span class="stream-cell-info">' + i18n_global_speed + '\uff1a<em class="stream-speed"></em></span>' +
					'	<span class="stream-cell-info">' + i18n_global_already_upload + '\uff1a<em class="stream-uploaded"></em></span>' +
					'	<span class="stream-cell-info">' + i18n_global_surplus_time + '\uff1a<em class="stream-remain-time"></em></span>' +
					'</div>',
		sTotalContainer = '<div id="#totalContainerId#" class="stream-total-tips">' +
			'	' + i18n_global_upload_total_progress + '\uff1a<span class="stream-process-bar total-bar"><span style="width: 0%;"></span></span>' +
			'	<span class="stream-percent">0%</span>\uff0c' + i18n_global_already_upload + '<strong class="_stream-total-uploaded">&nbsp;</strong>' +
			'	\uff0c' + i18n_global_upload_total_file_size + '<strong class="_stream-total-size">&nbsp;</strong>' +
			'</div>',
	...
	StreamProvider.prototype = {
		...
		renderUI : function(a) {
			...
			///////////////////////////
			this.get("dragAndDropAreaCopy") && !this.get("dragAndDropAreaCopy").nodeType && this.set("dragAndDropAreaCopy", document.getElementById(this.get("dragAndDropAreaCopy"))); 
		},
		bindUI : function() {
		...
		bindDropArea : function() {
			...
			var b = this.get("dragAndDropAreaCopy");
			...
			null !== b	&& (fAddEventListener(b, "drop", this.dropBinding),
					fAddEventListener(b, "dragenter", this.dropBinding),
					fAddEventListener(b, "dragover", this.dropBinding),
					fAddEventListener(b, "dragleave", this.dropBinding));
		},
		destroy : function() {
		...
		triggerEnabled : function() {
			...
			if (this.get("enabled") && null === this.buttonBinding){
				...
				this.bindSelectButton(), this.bindDropArea(), fRemoveClass(b, 'stream-disabled');
			}
				...
				null !== b && this.dropBinding != null && (fRemoveEventListener(b, "drop", this.dropBinding),
						fRemoveEventListener(b, "dragenter", this.dropBinding),
						fRemoveEventListener(b, "dragover", this.dropBinding),
						fRemoveEventListener(b, "dragleave", this.dropBinding),
						fAddClass(b, 'stream-disabled'));
			}				
		},
		updateFileList : function(a) {
	...
	function Main(cfg){
		...
		this.config = {
			...
			dragAndDropAreaCopy: cfg.dragAndDropAreaCopy,
	...
	Main.prototype = {
		...
		fileSelect : function(a) {
			...
			var onSelectStatus = this.get("onSelect") ? this.get("onSelect")(files) : this.onSelect(files);
			if("undefined" != typeof(onSelectStatus) && !onSelectStatus){
				return !1;
			}
